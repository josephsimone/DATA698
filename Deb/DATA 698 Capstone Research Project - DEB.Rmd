...
We can reject Null Hypothesis ($H_o$) since P-Values are all less than Tabulated value of 0.05 and accept the Alternate Hypothesis.
...
#oneway.test(formula = cpi_value ~ No_Jobs, data = cpi_emp_data_Combined, var.equal = T) 
...
library(multcomp)
# summary(glht(anova_one_way, linfct = mcp(cpi_value = "Tukey")))http://127.0.0.1:19984/rmd_output/4/#anova-tests
...
test <- chisq.test(table(cpi_emp_data_Combined$cpi_value, cpi_emp_data_Combined$No_Jobs))
test
