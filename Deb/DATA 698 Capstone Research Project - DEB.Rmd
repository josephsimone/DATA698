...
cpi_emp_data_Combined <- rbind(cpi_emp_data_USFoodBeverage_AK,
                        ...
                        mutate(No_Jobs = as.numeric(No_Jobs),
...
* Null Hypothesis: CPI Value has no effect on Number of Employment.

$H_o: \mu_1 = \mu_2 = \mu_3...= \mu_n$

* Alternative Hypothesis: CPI Value has effect on Number of Employment.
...
We can reject Null Hypothesis ($H_o$) since P-Value is `0.000209` whichis less than Tabulated value of 0.05 and thus accept the Alternate Hypothesis ($H_$).

## Linear Test
```{r, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE, echo=TRUE, results='show'}
# Levene's test is not appropriate with quantitative explanatory variables.
#car::leveneTest(cpi_emp_data_Combined$cpi_value ~ cpi_emp_data_Combined$No_Jobs)

#oneway.test(formula = cpi_value ~ No_Jobs, data = cpi_emp_data_Combined, var.equal = F) 

#statsr::inference(x=cpi_emp_data_Combined$cpi_value,
#          y=cpi_emp_data_Combined$No_Jobs,
#          est="mean",
#          null = 0,
#          alternative = "greater",
#          type="ht",
...
lm_cpi_emp_data_Combined <- lm(cpi_value ~ No_Jobs, data = cpi_emp_data_Combined)
summary(lm_cpi_emp_data_Combined)
```
...
```{r, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE, echo=TRUE, results='show'}
fit_labor <-  glm(cpi_value ~ No_Jobs, family=poisson(link='log'), data=cpi_emp_data_Combined)
...
Residuals = resid(stepwise), Treatment = cpi_emp_data_Combined$No_Jobs)
...
###One way ANOVA
anova_one_way <- aov(formula = cpi_value ~ No_Jobs, data = cpi_emp_data_Combined)
summary(anova_one_way)
plot(anova_one_way, 2)

anova_other_way <- aov(formula = No_Jobs ~ cpi_value, data = cpi_emp_data_Combined)
summary(anova_other_way)
plot(anova_other_way, 2)

###Pairwise ANOVA #no factors in the fitted model
#tuk <- TukeyHSD(anova_one_way, conf.level = 0.95)
#tuk
#plot(tuk, las = 1)

###Two way ANOVA
#anova_two_way_plot <- ggplot(cpi_emp_data_Combined, aes(x = cpi_value, y = No_Jobs, group=industry)) + 
#  geom_point(cex = 1.5, pch = 1.0,position = position_jitter(w = 0.1, h = 0)) +
#  labs(title = "2-way ANOVA")
#anova_two_way_plot
```
We can reject the null hypothesis at the ?? = 0.01 significance level (95% confidence). 

## T-Test
```{r, eval=TRUE, include=TRUE, message=FALSE, warning=FALSE, echo=TRUE, results='show'}
#test <- t.test(cpi_emp_data_Combined$No_Jobs, 
...
t.test(cpi_emp_data_Combined$No_Jobs, cpi_emp_data_Combined$cpi_value)

cor.test(cpi_emp_data_Combined$No_Jobs, cpi_emp_data_Combined$cpi_value)

#library(multcomp)
#summary(glht(anova_one_way, linfct = mcp(cpi_value = "Tukey")))
pairwise.t.test(cpi_emp_data_Combined$No_Jobs, cpi_emp_data_Combined$cpi_value,
                 p.adjust.method = "BH")
```
...
test <- chisq.test(cpi_emp_data_Combined$cpi_value, cpi_emp_data_Combined$No_Jobs)
test

#library(vcd)
#mosaic(cpi_value ~ No_Jobs,
#       direction = c("v", "h"),
#       data = cpi_emp_data_Combined,
#       shade = TRUE)

#library("graphics")
#cpi_emp_data_Combined %>%
#  select(Year,industry,state,cpi_value,No_Jobs) %>%
#  mosaicplot(shade = TRUE, las=2,
#             main = "CPI vs No Of Jobs")
```
